...
public class NOTA_CREDITO {
    ...
    public String GeneraXML(String idventa,String id_botica) {
        ...
        try {
            ...
            procedure = conex.prepareCall("{ call XML_NE_CABECERA (?,?) }");
            ...
            while (rs.next()) {
                ...
                cab.setResponsecode(rs.getString("ResponseCode"));
                cab.setOBSERVACIONES(rs.getString("OBSERVACIONES"));
            ...
            procedure1 = conex.prepareCall("SELECT * FROM XML_NE WHERE GRUPO=10 AND NE=1 ORDER BY POSICION");
            ...
            for (int i = 0; i < LstXML.size(); i++) {
                ...
                if (item.getVISUALIZA().contains("1001")) {
                    ...
                    item.setVISUALIZA(((CABECERA) listaVtasCabecera.get(0)).getVER_C1002().toString());
            ...
            /* DETALLE VENTA */
            /*****************/
            procedure2 = conex.prepareCall("{ call XML_NE_DETALLE (?,?) }");
            ...
            procedure3 = conex.prepareCall("SELECT * FROM XML_NE WHERE GRUPO=70 ORDER BY POSICION");
            ...
            for (int i = 0; i < listaXML.size(); i++)
            {
                if (listaXML.get(i).getVISUALIZA().toString().equals("1"))
                {
                    ...
                    if (!validadorCampo.equals(""))
                    {
                        ...
                        //String CampoCabecera = objproducto.RetornaDatoCabeceraNC(listaXML.get(i).getCAMPO().toString(),IDVTA,IDBOTI,TOTALTEXTO);
...
/*
           ...
           /*String VER_C6000 = objproducto.RetornaDatoNC("VER_C6000",IDVTA,IDBOTI,"2");
                ...
                for (int i = 0; i < listaXMLDetalle.size(); i++)
                {
                    if (listaXMLDetalle.get(i).getVISUALIZA().toString().equals("1"))
                    {
                        ...
                        if (!validadorCampo1.equals(""))
                        {
                            ...
                            //String CampoDetalle = objproducto.RetornaDatoDetalleNC(listaXMLDetalle.get(i).getCAMPO().toString(),IDVTA,IDBOTI);
